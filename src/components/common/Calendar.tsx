...
const Calendar = forwardRef<CalendarRef, CalendarProps>(
  (
    ...
    const monthData = useMemo(
      () => getMonthData(year, month, selectedDate, disabledDate, showAdjacentMonths),
      [year, month, selectedDate, disabledDate, showAdjacentMonths]
    );

    // 외부에서 사용할 메서드 정의
    const createNewDate = useCallback((targetYear: number, targetMonth: number) => {
    ...
    useImperativeHandle(ref, () => ({
      goToPrevMonth: () => {
        createNewDate(month === 0 ? year - 1 : year, month === 0 ? 11 : month - 1);
      },
      goToNextMonth: () => {
        createNewDate(month === 11 ? year + 1 : year, month === 11 ? 0 : month + 1);
      },
      goToPrevYear: () => {
              ...
              <tr key={weekIndex} className={styles['calendar-row']}>
                {week.map((cell, dayIndex) => {
                  ...
                  const cellClasses = [
                    ...
                    cell.isCurrentMonth ? styles['current-month'] : styles['adjacent-month'],
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        aria-label={cell.day > 0 ? `${formatDateToString(cell.date)} 선택` : ''}
                      >
                        {cellRender
